...
struct bitmap *bitmap_read(char *filename)
{
    ...
    bmp->file_type = read_word(bmpfile);
    ...
    bmp->n_colors = read_dword(bmpfile);
    bmp->n_colors_important = read_dword(bmpfile);

    /* palette data is bgr(a) */
    fseek(bmpfile, pal_offset, SEEK_SET);
    for (i = 0; i < bmp->n_colors; ++i) {
        int offset = i * 3;
    ...
    for (row = 0; row < bmp->height; ++row) {
        ...
        fseek(bmpfile,
              bmp->pixel_offset + (bmp->height - row - 1) * padded_width,
              SEEK_SET);
