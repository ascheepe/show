...
static void mda_show(struct bitmap *bmp)
{
    WORD column_offset;
    WORD row_offset;
    WORD column;
...
static void cga_show(struct bitmap *bmp)
{
    /*
...
static void ega_show(struct bitmap *bmp)
{
    /*
...
static void vga_show(struct bitmap *bmp)
{
    WORD column_offset;
    WORD row_offset;
    WORD row;
    ...
    for (row = 0; row < bmp->height; ++row) {
        ...
        maybe_exit();
        source = bmp->image + row * bmp->width;
        destination = vga_vmem_ptr(column_offset, row + row_offset);
